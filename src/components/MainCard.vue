...
<template>
	<div v-if="!loadingStatus" class="flex justify-center items-center flex-col text-[#351404]">
		...
		<div class="text-20px my-5px font-light">
			当前气温 {{ Math.round(weather.realtime.temperature) }}℃
		</div>
		...
		<div :style="AQIStyles" class="px-7px py-1px rounded my-5px">
			空气质量 {{ weather.realtime.air_quality.aqi.chn }}
...
@font-face {
	...
	src: url('../assets/FangZhengSuXinShiLiuKaiJianTi-1.ttf') format('truetype');
	font-weight: normal;
	font-style: normal;
}
